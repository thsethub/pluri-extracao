    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Nao foi possivel consultar a questao.";
      setError(message);
    } finally {
      setLoading(false);
    }
    ...
    <AppLayout>
      <div className={styles.container}>
        <div className={styles.header}>
          <h1>Consulta</h1>
          <p>
            Busca administrativa por ID da questao com o mesmo payload usado no
            classificar.
          </p>
        </div>
              ...
              <div className={styles.sideCard}>
                ...
                <div className={styles.sideSection}>
                  ...
                  <p className={styles.helperText}>
                    Consulta somente visual (nao salva classificacao).
                  </p>
                  ...
                  {questao.classificacao_manual ? (
                    <div className={styles.manualBox}>
                      <p>
                        ...
                        {questao.classificacao_manual.usuario_id}
                      </p>
                      <p>
                        <strong>Tipo de acao:</strong>{" "}
                        {questao.classificacao_manual.tipo_acao}
                      </p>
                      <p>
                        <strong>Data:</strong>{" "}
                ...
                <div className={styles.sideSection}>
                  <h4>Extracao</h4>
                  <p>
                    <strong>Tem extracao:</strong>{" "}
                    {questao.tem_extracao ? "Sim" : "Nao"}
                  </p>
                  <p>
                    <strong>Similaridade:</strong> {questao.similaridade ?? "-"}
                  </p>
                  <p>
                    <strong>Classificacao extracao:</strong>{" "}
                    {questao.classificacao_extracao?.length
                      ? questao.classificacao_extracao.join(" | ")
                      : "-"}
                  </p>
                  <p>
                    <strong>Nao enquadrada:</strong>{" "}
                    {questao.classificacao_nao_enquadrada?.length
                      ? questao.classificacao_nao_enquadrada.join(" | ")
                      : "-"}
                  </p>
                </div>
              </div>
