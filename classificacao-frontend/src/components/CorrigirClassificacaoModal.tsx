...
import { Search, X, ChevronDown, ChevronRight, CheckCircle, AlertTriangle, Pencil } from 'lucide-react';
...
export default function CorrigirClassificacaoModal({
    ...
    const disciplinas = Array.from(new Set(filtrados.map(m => m.disciplina))).sort();

    const modulosPorDisciplina = (disc: string) => filtrados.filter(m => m.disciplina === disc);
    ...
    const isDisciplinaExpandida = (disc: string) => buscaLower !== '' || expandidas.has(disc);
            ...
            <div className={styles.modal}>
                ...
                <div className={styles.searchArea}>
                    <div className={styles.searchWrap}>
                        ...
                        />
                        {busca && (
                            <button className={styles.clearSearch} onClick={() => setBusca('')} aria-label="Limpar busca">
                                <X size={14} />
                            </button>
                        )}
                    </div>
                        ...
                        <div className={styles.selectedTags}>
                            {selecionados.map(s => (
                                <span key={s.id} className={styles.selectedTag}>
                                    {s.modulo}
                                    <button onClick={() => toggleModulo(s)} className={styles.removeTag} aria-label="Remover">
                                        <X size={10} />
                                    </button>
                                </span>
                            ))}
                ...
                <div className={styles.content}>
                    {loadingModulos ? (
                        ...
                        <div className={styles.emptyState}>
                            <Search size={32} />
                            <p>Nenhum módulo encontrado para "<strong>{busca}</strong>"</p>
                        </div>
                    ) : (
                        <div className={styles.disciplinasList}>
                            {disciplinas.map(disc => {
                                ...
                                const selecionadosNaDisc = selecionados.filter(s => s.disciplina === disc).length;
                                return (
