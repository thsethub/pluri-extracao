...
def atualizar_lote():
    ...
    for q in questoes[:10]:  # Mostra primeiras 10
        ...
        print(f"{q.questao_id:<10} {q.questao_id_trieduc:<12} {len(q.enunciado_atual):<18} {q.tamanho_texto_base:<15} {len(enunciado_novo):<15}")
    ...
    for i, q in enumerate(questoes, 1):
        try:
            ...
            if imagens_s3:
                print(f"   📸 {len(imagens_s3)} imagem(ns) enviada(s) para S3 na questão {q.questao_id}")

            # Gera novo enunciado com texto_base já com URLs S3
            enunciado_novo = integrar_texto_base(texto_base_processado, q.enunciado_original)
            
            # Atualiza no banco
            db.execute(update_query, {
                "novo_enunciado": enunciado_novo,
            ...
            if i % 10 == 0 or i == len(questoes):
                print(f"   Processadas: {i}/{len(questoes)} | Sucesso: {sucesso} | Erros: {erros}")
    ...
    print(f"📈 Média de caracteres por questão: {chars_adicionados // sucesso if sucesso > 0 else 0:,}")
